...
dotenv.config(); // .env 내용을 process.env에 로드
...
const getTopRatedMoviesResponse = async () => {
	const config = {
		method: "GET",
...
const getSingleMovieResponse = async (movie_id) => {
	const config = {
		method: "GET",
...
const getNowPlayingTopMoviesResponse = async () => {
	const config = {
		method: "GET",
	...
	try {
		...
		let max_vote_average_movie = movies[0];
		for (const movie of movies) {
			max_vote_average_movie =
				max_vote_average_movie["vote_average"] > movie["vote_average"]
					? max_vote_average_movie
					: movie;
		}
		console.log(max_vote_average_movie);

		// const max_vote_average_movie = movies.reduce((prev, curr) =>
