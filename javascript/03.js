...
// 화살표 함수 정의 코드
const sumPositiveNumbersArrow = (nums) => {
	let sum = 0;
	for (let num of nums) {
		if (num > 0) sum += num;
	}
...
function getEvenNumbers(nums) {
	let array = [];
	for (let num of nums) {
		if (num % 2 == 0) array.push(num);
	}
	return array;
}

// 화살표 함수 정의 코드
const getEvenNumbersArrow = (nums) => {
	let array = [];
	for (let num of nums) {
		if (num % 2 == 0) array.push(num);
	}
...
function getHighScoreStudents(students) {
	...
	for (let student of students) {
		if (student["score"] >= 80) array.push(student["name"]);
	}
	return array;
}
...
function getLowStockProducts(products) {
	...
	for (let product of products) {
		if (product["stock"] < 10) array.push(product["name"]);
	}
	return array;
}
...
function getLongWords(words) {
	...
	for (let word of words) {
		if (word.length >= 5) strArray.push(word);
	}
	return strArray;
}
...
function transformNumbers(nums) {
	let numArray = [];
	for (let num of nums) {
		let data = num > 10 ? num : num % 2 == 0 ? num * 2 : num + 3;
		numArray.push(data);
	}
	return numArray;
}
	...
	for (let student of students) {
		let grade =
			student["score"] >= 90
				...
				? "C"
				: "D";
		evaluation[grade].push(student["name"]);
	}
	return evaluation;
}
...
function countElements(elements) {
	...
	for (let element of elements) {
		if (!cntObject.hasOwnProperty(element)) cntObject[element] = 1;
		else cntObject[element] += 1;
	}
	return cntObject;
}

// 화살표 함수 정의 코드
const countElementsArrow = (elements) => {
	let cntObject = {};
	for (let element of elements) {
		if (!cntObject.hasOwnProperty(element)) cntObject[element] = 1;
		else cntObject[element] += 1;
