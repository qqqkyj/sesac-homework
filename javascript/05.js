...
const border = "=".repeat(terminalWidth); //출력 구분
...
// TODO: 상품 종류 목록 조회
const getProductsCategoryList = async () => {
...
// TODO: Beef 요리법(recipes) 조회
const searchRecipes = async (recipe) => {
	const queryParams = new URLSearchParams({ q: recipe });
	const res = await axios.get(`${BASE_URL}/recipes/search?${queryParams}`);
	const data = res["data"];
	//console.table(Object.entries(data));
	console.log(data);
};
...
const sortProducts = async (a, b) => {
	const queryParams = new URLSearchParams({ sortBy: a, order: b });
...
const getCartsByAUser = async (userId) => {
